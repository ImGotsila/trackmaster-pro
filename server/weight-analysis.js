        ...
        db.all("SELECT * FROM cod_weight_rules WHERE isActive = 1", (err, rules) => {
            ...
            db.all("SELECT id, trackingNumber, shippingCost, codAmount, raw_data, customerName, phoneNumber, zipCode, importDate FROM shipments WHERE codAmount > 0", (err, rows) => {
                ...
                Object.keys(codGroups).sort((a, b) => parseFloat(a) - parseFloat(b)).forEach(codKey => {
                    ...
                    // Find Mode Weight (Auto-detect logic)
                    let modeWeight = null;
                    ...
                    let minValidWeight = modeWeight;
                    let maxValidWeight = modeWeight;
                    const rule = ruleMap[cod];

                    if (rule) {
                        // Use Manual Rule
                        minValidWeight = rule.minWeight;
                        maxValidWeight = rule.maxWeight;
                    }
                    ...
                    group.orders.forEach(order => {
                        const w = order.weight;
                        let isNormal = false;

                        if (minValidWeight !== null && maxValidWeight !== null) {
                            if (w >= minValidWeight && w <= maxValidWeight) {
                                isNormal = true;
                            }
                        } else {
                            // Fallback if no mode found (rare)
                            isNormal = true;
                        }

                        if (isNormal) {
                            normalCount++;
                        } else if (w < minValidWeight) {
                            underWeightCount++;
                            outliers.push({
                                ...order,
                                type: 'UNDERWEIGHT',
                                expectedWeight: modeWeight, // Show mode reference
                                ruleMin: minValidWeight,
                                ruleMax: maxValidWeight,
                                expectedCost: modeCost,
                                diff: parseFloat((w - minValidWeight).toFixed(3))
                            });
                        } else {
                            overWeightCount++;
                            outliers.push({
                                ...
                                ruleMax: maxValidWeight,
                                expectedCost: modeCost,
                                diff: parseFloat((w - maxValidWeight).toFixed(3))
                            });
                        }
                    });
                    ...
                    summary.push({
                        ...
                        ruleApplied: !!rule,
                        minValidWeight: minValidWeight,
                        maxValidWeight: maxValidWeight,
                        modeCost: modeCost,
                ...
                csv += 'COD,Mode Weight,Min Valid,Max Valid,Mode Cost,Total,Normal,Underweight,Overweight\n';
                summary.forEach(s => {
                    csv += `${s.cod},${s.modeWeight},${s.minValidWeight},${s.maxValidWeight},${s.modeCost},${s.totalOrders},${s.normalCount},${s.underWeightCount},${s.overWeightCount}\n`;
                });
