...
import { LayoutGrid, List, Scale, ArrowUp, ArrowDown, CheckCircle, AlertTriangle, ShieldAlert, ArrowRight } from 'lucide-react';
...
interface AnalysisData {
    ...
    allOutliers: Outlier[]; // Added to store flattened list of all outliers
}
    ...
    const fetchData = async () => {
        ...
        if (window.location.hostname.includes('github.io')) {
            const mockData: AnalysisData = {
                summary: [
                    { cod: 199, totalOrders: 150, modeWeight: 1.0, modeCost: 25, underWeightCount: 5, overWeightCount: 12, normalCount: 133, weightDistribution: { '1.0': 133, '0.5': 5, '2.0': 12 }, costDistribution: { '25': 140, '35': 10 }, outliers: [] },
                    { cod: 299, totalOrders: 80, modeWeight: 2.0, modeCost: 35, underWeightCount: 2, overWeightCount: 8, normalCount: 70, weightDistribution: { '2.0': 70, '1.0': 2, '5.0': 8 }, costDistribution: { '35': 75, '45': 5 }, outliers: [] }
                ],
                totalAnalyzed: 230,
                allOutliers: []
            };
        ...
        try {
            ...
            // Flatten all outliers for global table
        ...
        try {
            const res = await fetch('/api/weight-verification', {
                ...
                body: JSON.stringify({
                    ...
                    weight: row.weight,
                    normalWeight: row.expectedWeight,
                    codAmount: row.cod,
            ...
            if (res.ok) {
                // Optional: Remove from local state or show success
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {data.summary.map(group => (
                            <div
                                ...
                                className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm hover:shadow-md transition-all cursor-pointer group relative overflow-hidden"
                            >
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-slate-500">Normal Weight</span>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-indigo-100 shadow-sm bg-gradient-to-r from-indigo-50 to-white">
                        <div className="flex flex-col md:flex-row justify-between gap-6">
                            ...
                            <div className="flex gap-8">
                                <div>
                                    <div className="text-xs font-bold uppercase text-slate-400 mb-1">Normal Weight</div>
                                    <div className="text-2xl font-bold text-emerald-600">{activeGroup.modeWeight} kg</div>
                                </div>
                                <div>
                                    <div className="text-xs font-bold uppercase text-slate-400 mb-1">Normal Cost</div>
                                    <div className="text-2xl font-bold text-slate-700">฿{activeGroup.modeCost}</div>
                                </div>
                                <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                        ...
                                        <Bar dataKey="count">
                                            {Object.entries(activeGroup.weightDistribution)
                                                ...
                                                .map((entry, index) => (
                                                    <Cell
                                                        key={`cell-${index}`}
                                                        fill={entry.weight === activeGroup.modeWeight ? '#10b981' : (entry.weight < activeGroup.modeWeight ? '#fbbf24' : '#f43f5e')}
                                                    />
                                                ))}
                                        </Bar>
                                    </BarChart>
                            ...
                            <div className="flex justify-center gap-4 mt-4 text-xs font-medium">
                                ...
                                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-emerald-500 rounded-sm"></span> Normal</span>
                        ...
                        <div className="space-y-4">
                            <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
                                ...
                                <p className="text-xs text-amber-700 leading-relaxed">
                                    Orders lighter than {activeGroup.modeWeight}kg.
                                    Might be correct (small items) or input errors.
                                </p>
                            </div>
                            <div className="bg-rose-50 p-4 rounded-xl border border-rose-100">
                                ...
                                <p className="text-xs text-rose-700 leading-relaxed">
                                    Orders heavier than {activeGroup.modeWeight}kg.
                                    Often indicates a larger package or combined items.
                                </p>
                            </div>
                        </div>
                    </div>
